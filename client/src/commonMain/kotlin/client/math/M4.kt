...
@JvmInline
value class M4(val array: FloatArray) {
    companion object {
        fun m4Identity() = M4(
            floatArrayOf(
                1f, 0f, 0f, 0f,
    ...
    inline var m00: Float
        get() = array[0]
        set(x) {
            array[0] = x
        }

    inline var m01: Float
        get() = array[1]
        set(x) {
            array[1] = x
        }

    inline var m02: Float
        get() = array[2]
        set(x) {
            array[2] = x
        }

    inline var m03: Float
        get() = array[3]
        set(x) {
            array[3] = x
        }

    inline var m10: Float
        get() = array[4]
        set(x) {
            array[4] = x
        }
    ...
    fun multiply(x: Float, y: Float, z: Float) = V3.create(
        m00 * x + m10 * y + m20 * z + m30,
        m01 * x + m11 * y + m21 * z + m31,
        m02 * x + m12 * y + m22 * z + m32,
    )

    fun multiply(v: V3) = V3.create(
